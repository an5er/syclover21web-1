...
func get_server(value string) (result bool) {
	result = false
	if servers == value {
		result = true
	}
...
func get_title(value string) (result bool) {
	result = false
...
func get_body(value string) (result bool) {
	result = strings.Contains(bodys, value)
	return result
}

func get_banner(value string) (result bool) {
	result = false
...
func get_head(value string) (result bool) {
		...
		for i = 0; i < len(configObj.Rules); i++ {
			if len(configObj.Rules[i]) >= 2 {
				...
				for j = 0; j < len(configObj.Rules[i]); j++ {
					arr = append(arr, check_model(configObj.Rules[i][j].Match, configObj.Rules[i][j].Content, url))
				}
				...
				if check_model(configObj.Rules[i][0].Match, configObj.Rules[i][0].Content, url) {
					resulte_write(url, configObj.Product)
				}
